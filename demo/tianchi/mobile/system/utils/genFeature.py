...
def genFeat(df_fd, df_ld, label_date, nDay):
    ...
    uic = pd.DataFrame(dict(
        ...
        uid_behav_cnt = fd_group_ui.behavior_type.size(),
            # uid的行为总计数
        uid_buy_cnt = df_bd[df_bd.behavior_type == 4].groupby(['user_id', 'item_id']).behavior_type.size()
            # uid的购买计数
    )).reset_index()
    ...
    ucc = pd.DataFrame(dict(
        ...
        uc_behav_cnt = fd_group_uc.behavior_type.size(),
            # uid的行为总计数
        uc_buy_cnt = df_bd[df_bd.behavior_type == 4].groupby(['user_id', 'item_category']).behavior_type.size()
            # uid的购买计数
    )).reset_index()
...
def main():
    ...
    label_date = '2014-12-19'
    nDay = 4
    while (label_date != '2014-12-19'):
        df_fd, df_ld, df_bd = extractTargetDatasetOf(df_train, label_date, nDay)
        ## 提取特征
        df_feat = genFeat(df_fd, df_ld, df_bd, label_date, nDay)
        ## 获取标签
