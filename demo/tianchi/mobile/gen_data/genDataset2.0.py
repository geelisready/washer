...
PATH_OF_DATAOUT = 'F:/codeGit/my project/python/dataset/tianchi/offline/'
...
def main_offline():
	...
	while(labelTimeList[0] != '2014-12-19'):
		...
		if cnt == 0:
			sampleSet.to_csv(PATH_OF_DATAOUT + 'dataset' + '_' + str(len(featTimeList)) + 'days.csv', mode = 'w', index = False)
		else: 
			sampleSet.to_csv(PATH_OF_DATAOUT + 'dataset' + '_' + str(len(featTimeList)) + 'days.csv', mode = 'a', index = False, header = False)
		cnt = cnt + 1
...
def main_online():
	...
	store.saveToCsv(featureSet, labelTimeList[0] + '_' + str(len(featTimeList)) + 'days_online.csv')
